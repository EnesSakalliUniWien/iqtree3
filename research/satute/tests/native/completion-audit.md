...
| Formula variants and FDR are emitted | Native output includes `dominant` and `eigenvalue_weighted`. Pooled branch p-values receive Benjamini-Yekutieli adjustment in two separate formula families; rate-category rows are excluded. The Python comparison keeps `all_unweighted` as an offline baseline. | Complete. |
