# Eigenvalue-Weighted Branch Saturation Testing

This Markdown file is intentionally only a pointer. The working manuscript is
the LaTeX source:

```text
doc/satute-wiki/short-addendum-paper.tex
```

The compiled PDF is:

```text
doc/satute-wiki/short-addendum-paper.pdf
```

The earlier Markdown draft has been superseded because it overinterpreted a
reduced development run. The current manuscript draft uses the SatuTe notation
and treats the reduced simulation output only as verification of software
wiring.
