...
This construction tests whether the dominant surviving spectral direction links \(T_A\) and \(T_B\). Under a general time-reversible (GTR) model, \(\lambda_1\), \(\lambda_2\) and \(\lambda_3\) are usually distinct. Their eigenvectors \(h_i\) describe different directions in nucleotide state space, while their eigenvalues describe different decay rates along branch \(AB\). A statistic that uses only \(C_{\partial 1}\) can omit signal carried by \(h_2\) or \(h_3\). The weighted statistic tested here allows non-dominant directions to contribute only through the relative decay implied by the fitted model.
...
The weighted statistic in \cref{eq-weighted-coherence} separates direction from persistence. For short branches, \(w_i(t)\) remains close to one and multiple directions can contribute. For long branches, \(w_i(t)\) approaches zero for faster modes and the statistic approaches the dominant SatuTe coefficient.
...
After the JC baseline reproduces the published behavior, the extension can be evaluated under GTR. The comparison is paired. One simulated alignment, one random seed, one analysis scenario and one target branch define a replicate. On that same replicate, the analysis computes the published dominant coefficient \(C_{\partial 1}\) and the eigenvalue-weighted coefficient \(C_{\partial}^{\lambda}(t)\). The same one-sided normal test, the same \(\alpha\), and the same \(m_A m_B\) Bonferroni threshold are then applied to both statistics. The endpoints are type I error under subtree independence, power under known informative branches, behavior under maximum-likelihood branch-length estimation, and inflation under maximum-likelihood topology selection with and without the Bonferroni correction.

The paired design changes the reporting. Power curves still show the fraction of replicates called informative, but each parameter combination should also include a discordance table: dominant-only, weighted-only and agreement. McNemar tests on paired decisions and paired summaries of \(Z_{\lambda}(t)-Z_1\) or \(-\log_{10}P_{\lambda}+\log_{10}P_1\) compare formulas on the same simulated alignments and remove simulation noise shared by both formulas.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.22\textwidth}p{0.31\textwidth}p{0.35\textwidth}}
    ...
    Replicate unit & One alignment, one scenario, one target branch & Same replicate for both formulas \\
    Test threshold & Same \(\alpha\), same Bonferroni rule & Same \(\alpha\), same Bonferroni rule \\
    Statistics & \(C_{\partial 1}\) & \(C_{\partial 1}\), \(C_{\partial}^{\lambda}(t)\) \\
...
The curated biological rerun applies the same extension to the manuscript 16S rRNA Tree-of-Life dataset. The dataset is the rRNA-based 3D tree with \(1{,}947\) alignment sites, a fitted GTR+F+G4 model, four rate categories and 36-site sliding windows. The focal branches are the internal branch leading to Eukaryota and the external branch leading to yeast. The comparison keeps the published SatuTe rerun, the native dominant-mode calculation and the eigenvalue-weighted statistic.

For the branch leading to Eukaryota, the published SatuTe rerun marks \(822\) of \(1{,}912\) windows as saturated. The native dominant calculation marks \(895\) windows, whereas the eigenvalue-weighted statistic marks \(699\) windows. For the yeast branch, the corresponding counts are \(70\), \(83\) and \(29\) saturated windows. The difference is therefore concentrated in the same sliding-window setting used by the original paper, rather than in a simulated example.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{results/biological/enhanced_sliding_window_rerun_20260619_150104/comparison_to_manuscript/original_vs_enhanced_sliding_window_curves.pdf}
  \caption{\textbf{Published SatuTe and eigenvalue-weighted sliding-window curves for the 16S rRNA Tree-of-Life example.} The curves compare the manuscript SatuTe rerun with the native dominant calculation and the eigenvalue-weighted statistic for the branches leading to Eukaryota and yeast.}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{results/biological/enhanced_sliding_window_rerun_20260619_150104/comparison_to_manuscript/original_vs_enhanced_saturated_windows.pdf}
  \caption{\textbf{Number of saturated sliding windows in the 16S rRNA Tree-of-Life example.} The eigenvalue-weighted statistic changes the number of windows classified as saturated while using the same 36-site windows and the same branch targets.}
\end{figure}

A second biological rerun uses an independent implementation that does not read SatuTe component files or SatuTe result trees. It recomputes the sliding-window scores from the IQ-TREE treefile, model report, site-probability output and the original alignments. This run covers both Tree-of-Life examples from the manuscript: the ribosomal protein alignment on the protein-based 2D tree under LG+G4, and the 16S rRNA alignment on the rRNA-based 3D tree under GTR+G4. In the protein 2D tree, the branch leading to Eukaryota has \(2{,}561\) windows, with \(138\) saturated windows in the published SatuTe rerun and \(5\) in the eigenvalue-weighted calculation. The yeast branch also has \(2{,}561\) windows, with counts \(54\) and \(8\). In the 16S rRNA 3D tree, the branch leading to Eukaryota has \(1{,}912\) windows, with counts \(822\) and \(699\). The yeast branch has \(1{,}912\) windows, with counts \(70\) and \(29\).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{results/biological/independent_enhanced_sliding_window_20260619_151135/comparison_to_manuscript/published_vs_eigenvalue_weighted_curves.pdf}
  \caption{\textbf{Independent eigenvalue-weighted sliding-window curves for the protein 2D and 16S rRNA 3D Tree-of-Life examples.} The independent calculation compares the published SatuTe sliding-window score with the eigenvalue-weighted statistic on the same 36-site windows and target branches.}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{results/biological/independent_enhanced_sliding_window_20260619_151135/comparison_to_manuscript/published_vs_eigenvalue_weighted_saturated_windows.pdf}
  \caption{\textbf{Saturated-window counts in the independent biological comparison.} Counts are shown for the branch leading to Eukaryota and the branch leading to yeast in the protein-based 2D tree and the 16S rRNA-based 3D tree.}
\end{figure}
...
No full-scale simulation data are reported in this draft. The SatuTe source paper, the manuscript archive and public repositories are available from the cited sources. The local repository contains reduced simulation outputs used only to verify command wiring, branch selection and paired formula evaluation, and it contains the curated biological reruns used for the sliding-window figures.
