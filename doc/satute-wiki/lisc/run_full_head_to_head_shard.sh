...
EVONAPS_BRANCH_LENGTHS="${EVONAPS_BRANCH_LENGTHS:-${ROOT_DIR}/doc/satute-wiki/sources/evonaps/evonaps_16taxon_branch_lengths.tsv}"
...
PYTHONPATH="${ROOT_DIR}${PYTHONPATH:+:${PYTHONPATH}}" "${PYTHON_BIN}" \
  "${ROOT_DIR}/doc/satute-wiki/head_to_head_satute_simulations.py" \
  --iqtree "${IQTREE_BIN}" \
  --seqgen "${SEQGEN_BIN}" \
  --simulator seq-gen \
